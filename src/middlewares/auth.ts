...
export const getUser = () => (req: RequestCustom, res: Response, next: NextFunction) =>
  passport.authenticate('jwt', { session: false }, setUser(req, res, next))(req, res, next);

const setUser = (req: RequestCustom, res: Response, next: NextFunction) => async (err: any, user: IUser & boolean, info: any) => {
...
const setSpace = (req: RequestCustom, res: Response, next: NextFunction) => async (err: any, space: ISpace & boolean, info: any) => {
  console.log(space);
  req.space = space;
  // req.query.organization = space?.organization;
  return next();
};

export const handleSpaceJwt = () => (req: RequestCustom, res: Response, next: NextFunction) =>
